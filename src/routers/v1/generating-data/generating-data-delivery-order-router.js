        ...
        db.get().then(db => {
            ...
            manager.getAllData(filter)
                .then(list => {
                    ...
                    if (dateFrom && dateTo) {
                        response.xls(`Laporan Surat Jalan -  ${moment(new Date(dateFrom)).format(dateFormat)} -  ${moment(new Date(dateTo)).format(dateFormat)}.xlsx`, data, options);
                    } else {
                        response.xls(`Laporan Surat Jalan -  ${moment(new Date()).format(dateFormat)}.xlsx`, data, options);
                    }
                })
                .catch(e => {
